...
import com.online_exams.university_project.repositories.HelpMessageRepository;
import com.online_exams.university_project.services.email.MailService;
...
public class HelpService {
    private HelpMessageRepository helpRepository;
	...
	public boolean deleteMessage(long id) {
		Optional<HelpMessage> messageO= this.helpRepository.findById(id);
		if(messageO.isPresent()) {
			this.helpRepository.delete(messageO.get());
			return true;
		}else return false;
	}
    public void submitHelpRequest(HelpMessage message){
        this.helpRepository.save(message);
    }
    
    public boolean replyToHelpMessage(long messageId, String context) {
        try {
            HelpMessage helpMessage = this.helpRepository.getById(messageId);
    ...
    private String buildEmail(String context) {
        return "<div style=\"font-family:Helvetica,Arial,sans-serif;font-size:16px;margin:0;color:#0b0c0c\">\n" +
                ...
                "                      <span style=\"font-family:Helvetica,Arial,sans-serif;font-weight:700;color:#ffffff;text-decoration:none;vertical-align:top;display:inline-block\">Online Examination WebSite</span>\n" +
                "                    </td>\n" +
                "                  </tr>\n" +
                "                </tbody></table>\n" +
                "              </a>\n" +
